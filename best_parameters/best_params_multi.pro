PRO best_params_multi, inpfile, planet, CONSTRAINED=con, uncon=uncon
; File to find best starting tcenter;;;;maybe(, inc, and ar*)
  ...
  IF KEYWORD_SET(con) THEN BEGIN
     ...
     savfile2='./WLresults/best_params/constrained/'+planet+'/'+inpfile+'.sav'
     SAVE, filename=savfile2, final_results, starting_params
  ENDIF
  IF KEYWORD_SET(uncon) THEN BEGIN
        
     infile2='./WLresults/best_params/constrained/'+planet+'/'+uncon+'.sav'
     RESTORE, infile2
 

;;;;;;; Read in error on period
     per_file='./eclipse_inputs/period_err/'+planet+'.dat'
