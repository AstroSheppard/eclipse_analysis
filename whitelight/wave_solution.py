...
def wave_solution(visit, dir, plotting=False, savename=False, phase=False, transit=False):
    ...
    data=glob.glob('../data_reduction/zapped2017/'
                   + visit +'/'+ dir +'/final/*.zap.fits')

    exp=orbits(data, phase=phase, transit=transit)[0] # may have phase mixed up
    
    # Read in data, and normaliza the spectrum
    data=glob.glob('../data_reduction/zapped2017/'+ visit +'/'+ dir +'/final/'
                   +"%03d"%exp+'.zap.fits')
    spec=np.sum(fits.open(data[0])[0].data, axis=0) # make sure axis is right
    err=np.sqrt(spec)
    # Normalize
    err=err/np.max(spec)
    ...
    if savename:
        ...
        try:
            ...
            current.to_csv('./wave_sol/wave_solution.csv', index=False)
        
        except IOError:
            wave_solution.to_csv('./wave_sol/wave_solution.csv', index=False)
        ...
        try:
            ...
            cur.to_csv('./wave_sol/wave_solution_extras.csv', index=False)
            print cur
        except IOError:
            extras.to_csv('./wave_sol/wave_solution_extras.csv', index=False)
...
if __name__ == '__main__':
    if len(sys.argv) < 4 or len(sys.argv) > 7:
        sys.exit('Run using [program.py] [planet] [visit #] [scan direction] [plot?] [savename?]')
    visit=sys.argv[1]+'/'+sys.argv[2]
    dir=sys.argv[3]
    if len(sys.argv) == 7:
        plotting=int(sys.argv[4])
        name=sys.argv[5]
        trans=int(sys.argv[6])
    elif len(sys.argv)== 6:
        plotting=int(sys.argv[4])
        name=sys.argv[5]
        trans=False
    ...
    wave=wave_solution(visit, dir, plotting=plotting, savename=name, transit=trans)
